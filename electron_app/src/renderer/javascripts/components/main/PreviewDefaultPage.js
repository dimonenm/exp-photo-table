...
function PreviewDefaultPage({ number, index, orientation, img, text, photoTableData, settings, setModalProperties }) {
    ...
    if (index[0] && img[0] && orientation[0] === 'vertical') {
        imagesArr.push((
            <>
                <div className="preview-default-page-img-vertical" onDoubleClick={dbClickSecondImgHandler} style={{ backgroundImage: `url(${img[0]})` }}></div>
    ...
    if (index[0] && img[0] && orientation[0] === 'horizontal') {
        imagesArr.push((
            <>
                <div className="preview-default-page-img" onDoubleClick={dbClickSecondImgHandler} style={{ backgroundImage: `url(${img[0]})` }}></div>
